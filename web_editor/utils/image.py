...
def image_process(source,
    ...
    if not source or ((not size or (not size[0] and not size[1]))
                      ...
                      and not colorize and not output_format):
    ...
    if size:
        if crop:
            ...
            if crop == 'top':
                ...
                center_y = 1
            image.crop_resize(max_width=size[0],
                              max_height=size[1],
                              center_x=center_x,
                              center_y=center_y)
        else:
...
def average_dominant_color(colors, mitigate=175, max_margin=140):
    ...
    for color in range(3):
        value = dominant_avg[color] / (
            brightest /
            mitigate) if brightest > mitigate else dominant_avg[color]
...
def image_fix_orientation(image):
    ...
    getexif = getattr(image, 'getexif', None) or getattr(
        image, '_getexif', None)  # support PIL < 6.0
    if getexif:
        exif = getexif()
        if exif:
...
def is_image_size_above(base64_source_1, base64_source_2):
    ...
    if not base64_source_1 or not base64_source_2:
        return False
    if base64_source_1[:1] in (b'P', 'P') or base64_source_2[:1] in (b'P',
                                                                     'P'):
        # False for SVG
        return False
